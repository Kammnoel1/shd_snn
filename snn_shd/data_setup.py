...
class SHDDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, data_path: Path, duration: float, n_steps: int, n_neurons: int):
    ...
    def __getitem__(self, idx: int):
        ...
        Returns:
          ...
          (n_steps, n_neurons) and label is a scalar tensor.
        """
        ...
        time_bins = np.linspace(
            0, self.max_time, num=self.n_steps + 1
        )  # Create n_steps bins
        bin_indices = np.clip(
            np.digitize(times, time_bins) - 1, 0, self.n_steps - 1
        )  # np.digitize is 1-based, substract 1 to get 0-based bins
        spike_matrix = torch.zeros(self.n_steps, self.n_neurons, dtype=torch.float32)
...
def create_dataloaders(
    ...
    n_neurons: int = config.IN_NEURONS,
    ...
    test_data = SHDDataset(
        data_path=test_path, duration=duration, n_steps=n_steps, n_neurons=n_neurons
    )
