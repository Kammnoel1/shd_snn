...
def over_time_loss(mem2_trace: torch.Tensor, labels: torch.Tensor) -> torch.Tensor:
    ...
    Args:
      mem2_trace: Output membrane potential trace, shape (NUM_TIMESTEPS, BATCH_SIZE, NUM_CLASSES).
      labels: Ground-truth class labels, shape (B,), dtype int64.

    Returns:
      Scalar cross-entropy loss.
    """
    logits, _ = mem2_trace.max(dim=0)  # (B, NUM_CLASSES) — per-class max over time
...
def regularization_loss(
    ...
    Args:
      spk1_trace: Hidden-layer spike trace, shape (T, B, NUM_HIDDEN).
    ...
    """
    _, batch_size, num_hidden = spk1_trace.shape
    mean_rate_per_neuron = spk1_trace.mean(dim=0)
    l1_term = torch.clamp(mean_rate_per_neuron - l1_thres, min=0) ** 2
    l1 = l1_strength / (batch_size + num_hidden) * l1_term.sum()

    total_count_per_sample = spk1_trace.sum(dim=(0, 2)) / num_hidden
